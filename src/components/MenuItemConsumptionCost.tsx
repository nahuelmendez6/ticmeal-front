...
import { useMenuItemConsumptionCost } from '../hooks/useMenuItemConsumptionCost';
import { 
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend,
  PieChart, Pie, Cell
} from 'recharts';

const MenuItemConsumptionCost: React.FC = () => {
  const { report, loading, error, fetchReport } = useMenuItemConsumptionCost();
  ...
  const chartData = useMemo(() => {
    ...
    return report.map(day => {
      ...
      day.details.forEach(detail => {
        dayData[detail.menuItemName] = detail.cost;
      });
      return dayData;
    });
  }, [report]);

  // Obtener lista única de items para generar las series del gráfico
  const menuItemKeys = useMemo(() => {
    if (!report) return [];
    const keys = new Set<string>();
    report.forEach(day => {
      day.details.forEach(detail => {
        keys.add(detail.menuItemName);
      });
    });
  ...
  const pieData = useMemo(() => {
    ...
    report.forEach(day => {
      day.details.forEach(detail => {
        totals[detail.menuItemName] = (totals[detail.menuItemName] || 0) + detail.cost;
      });
    });
    return Object.entries(totals)
      .map(([name, value]) => ({ name, value }))
      .filter(item => item.value > 0); // Filtrar items con costo 0 para el gráfico de torta
  }, [report]);
  ...
  return (
    <div className="mt-5">
      <h4 className="mb-3 text-primary-emphasis">Costo de Consumo por Ítem del Menú</h4>
      
      <form onSubmit={handleSearch} className="row g-3 mb-4 align-items-end bg-light p-3 rounded border">
        <div className="col-md-4 col-sm-6">
          <label htmlFor="menuStartDate" className="form-label fw-medium">Desde</label>
          <input 
            type="date" 
        ...
        <div className="col-md-4 col-sm-6">
          <label htmlFor="menuEndDate" className="form-label fw-medium">Hasta</label>
          <input 
            type="date" 
            className="form-control" 
            id="menuEndDate" 
      ...
      {report && (
        <>
              ...
              <div className="col-lg-8 mb-4 mb-lg-0">
                <div className="card border-light shadow-sm h-100">
                  <div className="card-body">
                    <h5 className="card-title text-muted mb-3">Evolución Diaria de Costos (Menú)</h5>
          ...
          <div className="accordion" id="menuConsumptionAccordion">
            {report.map((item) => (
            ...
            {report.length === 0 && !loading && (
              <div className="alert alert-info text-center">No se encontraron costos de consumo de menú para el rango seleccionado.</div>
            )}
          </div>
        </>
      )}
...
export default MenuItemConsumptionCost;